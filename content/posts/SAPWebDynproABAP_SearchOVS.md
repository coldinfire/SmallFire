...
![Web Dynpro OVS](/images/webdynpro/webdynproABAP/Portal30.png)
...
```ABAP
METHOD on_ovs_owner .
  TYPES: BEGIN OF ty_stru_input, "定义查询参数结构"
    codegruppe TYPE qcodegrp,
  END OF ty_stru_input.
  TYPES: BEGIN OF ty_stru_list,  "定义搜搜帮助返回结果字段列表"
    codegruppe TYPE qcodegrp,
    code TYPE qcode,
  ...
  DATA: ls_search_input  TYPE ty_stru_input,
        ls_select_list   TYPE ty_stru_list,
        lt_select_list   TYPE STANDARD TABLE OF ty_stru_list,
...
METHOD init_select_options .
  ...
  DATA:display_btn_cancel  TYPE abap_bool,
...
******** TODO exception handling
      ...
      IF gt_file IS NOT INITIAL.
        SELECT * INTO CORRESPONDING FIELDS OF TABLE lt_select_list
          FROM tj30t FOR ALL ENTRIES IN gt_file
         WHERE stsma = gt_file-user_stat_proc
           AND spras = sy-langu.
      ENDIF.
