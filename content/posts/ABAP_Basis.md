...
| :----- | :----------------------- | :--------------------- | :------ | :--------------------- | :--------- |
| C      | 字符串(定义文字，数字等) | 定义时指定字符个数     | D       | 日期(YYYYMMDD)         | 8字符      |
| N      | 数字字符串(字符串整数)   | 定义时指定字符个数     | T       | 时间(HHMMSS)           | 6字符      |
| X      | 十六进制                 | 定义时指定字节数       | F       | 浮点类型               | 8字节      |
| string | 变长字符串               | 长度不固定             | xstring | 变长字节序列类型       | 长度不固定 |
| P      | 组合数值类型(1-16)       | 定义时指定长度，小数位 | I       | 整数(非整数会四舍五入) | 4字节      |
