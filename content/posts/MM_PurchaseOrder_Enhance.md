...
METHOD IF_EX_ME_PROCESS_PO_CUST~PROCESS_ITEM.
    ...
    IF  SY-TCODE = 'ME21N' OR SY-TCODE = 'ME22N'  OR SY-TCODE = 'ME29N'
      ...
      ELSE .
        ...
        CLEAR LV_MARKS .
        CLEAR LW_KONV .
        READ TABLE LT_KONV INTO LW_KONV WITH KEY KPOSN = LW_ITEM-EBELP KSCHL = 'PB00' .
        IF SY-SUBRC <>  0 .
          CLEAR LW_KONV .
          READ TABLE LT_KONV INTO LW_KONV WITH KEY KPOSN = LW_ITEM-EBELP KSCHL = 'PBXX' .
          IF SY-SUBRC <> 0 .
            LV_MARKS = 'X' .
          ENDIF .
        ENDIF .
        CHECK LV_MARKS = '' .
        SELECT SINGLE KNUMH INTO LV_KNUMH FROM A901
            WHERE KSCHL = 'PB00'
        ...
        IF SY-SUBRC = 0 .
          ...
          IF SY-SUBRC = 0 .
            SELECT * INTO CORRESPONDING FIELDS OF TABLE LT_KONM
              ...
              WHERE KNUMH = LT_KONP-KNUMH 
              AND KOPOS = LT_KONP-KOPOS." AND KSTBM <= LW_ITEM-MENGE. "
            CLEAR LW_KONP .
            ...
            IF SY-SUBRC = 0 .
              ...
              IF SY-SUBRC = 0.
                SELECT * INTO CORRESPONDING FIELDS OF TABLE LT_KONM 
                  ...
                  FOR ALL ENTRIES IN LT_KONP
                  WHERE KNUMH = LT_KONP-KNUMH 
                  AND KOPOS = LT_KONP-KOPOS." AND KSTBM <= LW_EKPO-MENGE. "
              ENDIF.
              CLEAR LW_KONP .
              READ TABLE LT_KONP INTO LW_KONP INDEX 1 .
              SORT LT_KONM BY KNUMH KOPOS KSTBM DESCENDING."先取最大的，然后倒序，小于等于当前数量赋值"
              ...
              IF LW_EKPO-KBETR < LW_KONP-KBETR .
                ...
                LW_BAPICONDIT-SCALEBASIN = LW_KONP-STFKZ.  "KZBZG Teken:rekeneenheid "
                LW_bapicondit-scalebasin = 'C'.            "存在数量等级"
